...
export function SiteHeader({ breadcrumb }: { breadcrumb?: string }) {
  ...
  return (
    <header className="bg-background/95 border-border sticky top-0 z-50 border-b backdrop-blur">
      <div className="mx-auto flex max-w-[1600px] items-center gap-3 px-4 py-2.5 sm:px-6 lg:gap-5">
        ...
        {/* Меню — на одном уровне с логотипом */}
        <nav
        ...
        >
          {navItems.map((item) => (
            <a
              ...
              href={item.href}
              {...linkProps(item.href)}
              className="text-foreground hover:text-brand-green shrink-0 whitespace-nowrap text-[13px] font-semibold transition-colors xl:text-[15px]"
            >
              {item.label}
            </a>
          ))}
        ...
        {isHome && <SiteSearch className="flex min-w-0 flex-1 lg:hidden" />}

        <div className="ml-auto hidden shrink-0 items-center gap-3 lg:flex">
          <SiteSearch className="w-[150px] shrink-0 xl:w-[190px]" />
          <a
            href={`tel:${phone.replace(/[^+\d]/g, "")}`}
            className="hidden shrink-0 items-center gap-2 xl:flex"
          >
            <Phone className="text-muted-foreground size-5" strokeWidth={1.6} />
            <span className="flex flex-col leading-tight">
              <span className="text-muted-foreground text-[11px]">{phoneNote}</span>
          ...
          <a
            ...
            className="gradient-accent text-accent-foreground whitespace-nowrap rounded-full px-4 py-2.5 text-[14px] font-bold transition-transform duration-300 hover:-translate-y-0.5 hover:brightness-105"
          >
